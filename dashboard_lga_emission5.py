...
NC_FILE = "/mnt/scratch_lustre/duch/whe_dashboard/emission_nc/gridded_emissions_weekday_July_NSW_ACT_combined.nc"
...
# Additional UI: Allow multiple SA3s and export format
selected_regions = st.sidebar.multiselect("Select SA3 Region(s)", sorted(sa3_in_state), default=[selected_region])
export_format = st.sidebar.radio("Export Format", ["CSV", "NetCDF"])

# Create geometry mask for selected regions
selected_polygons = gdf_sa3[gdf_sa3["SA3_NAME16"].isin(selected_regions)].geometry.unary_union
mask = np.array([selected_polygons.contains(Point(x, y)) for x, y in zip(lon2d.ravel(), lat2d.ravel())], dtype=bool).reshape(lat2d.shape)

# Apply mask
species_data = ds[selected_species].values
masked_region_data = np.where(mask[None, :, :], species_data, np.nan)
# Time series (mean over all selected SA3s)
...
ax2.set_title(f"Diurnal Emission Pattern of {selected_species} in {', '.join(selected_regions)}")
ax2.set_xlabel("Hour of Day")
ax2.set_ylabel(f"{selected_species} (kg/hour)")
st.pyplot(fig2)

# --- Export Data ---
if st.checkbox("Enable Export for Selected Region(s)"):
    if export_format == "CSV":
        # Flattened CSV export
        hour_list, lat_list, lon_list, value_list = [], [], [], []
        ...
        st.download_button(
            label="Download Emission Data (CSV)",
            data=export_df.to_csv(index=False),
            file_name=f"{selected_species}_export.csv",
            mime="text/csv"
        )
    else:
        # NetCDF export
        # Create the dataset
        out_ds = xr.Dataset(
            {f"{selected_species}": (("time", "latitude", "longitude"), masked_region_data)},
            coords={"time": ds["time"], "latitude": lat, "longitude": lon}
        )

        # Directly generate bytes and wrap in BytesIO
        nc_bytes = out_ds.to_netcdf()
        nc_buffer = BytesIO(nc_bytes)

        # Create download button
        st.download_button(
            label="Download Emission Data (NetCDF)",
            data=nc_buffer,
            file_name=f"{selected_species}_export.nc",
...
#ax2.set_xlabel("Hour of Day")
#ax2.set_ylabel(f"{selected_species} (mol/s)")
#st.pyplot(fig2)
